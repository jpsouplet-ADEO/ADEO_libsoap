...
xmlbuilder_start_element(const xmlChar * element_name, int attr_count, xmlChar ** keys, xmlChar ** values, void *userData)
{
  ...
  //printf("===>Start  Element name = %s with parent %s\n",element_name, parent);
...
xmlbuilder_characters(const xmlChar * element_name, const xmlChar * chars, void *userData)
{
  ...
  //printf("===>xmlbuilder_characters  Element name = %s with parent:%s xmlchars:%s\n",element_name, parent, chars);
  xmlNodeSetContent(parent,chars);
...
#ifdef USE_XMLSTRING
    xmlStrPrintf(buffer, 1054, BAD_CAST _SOAP_MSG_TEMPLATE_NEW,
                 soap_env_ns, soap_env_enc, soap_xsi_ns,soap_xsd_ns);
#else
    sprintf(buffer, _SOAP_MSG_TEMPLATE_NEW,
            soap_env_ns, soap_env_enc, soap_xsi_ns,soap_xsd_ns);
#endif

  if (!(env = xmlParseDoc(buffer)))
    return herror_new("soap_env_new",
                      XML_ERROR_PARSE, "Can not parse xml");
...
soap_env_get_method(SoapEnv * env)
{
  ...
  methodchild = soap_xml_get_children(body);
  if (methodchild == NULL)  log_verbose1("children is NULL -> NO method");
  return methodchild;
}
