...
/* Initialise a zero serverSockAddr */
  ...
  // 20171009 Adaptation SSL SHA2 - DEBUT ---
  //if ((long)hostAddr != (long)-1)
  if ((in_addr_t)hostAddr != (in_addr_t)-1)
  // 20171009 Adaptation SSL SHA2 - FIN ---
  {
  ...
  else /* Si on a donné un nom ou une adresse DNS */
  {
    ...
    if (serverHostEnt == NULL)
       return herror_new("hsocket_open", HSOCKET_ERROR_GET_HOSTNAME,
                      "Socket error get hostname (%d) (%s)", h_errno,hstrerror(h_errno));
 ...
 /* Creation de la socket et retourne un descripteur de fichier
  ...
  if ((dsock->sock = socket(AF_INET, SOCK_STREAM, 0)) < 0)
    return herror_new("hsocket_open", HSOCKET_ERROR_CREATE,
                      "Socket error create (%d) (%s)", errno,strerror(errno));

  log_verbose4("Opening %s://%s:%i", ssl ? "https" : "http", hostname, port);

  /* connect to the server */
  if (connect(dsock->sock, (struct sockaddr *) &serverSockAddr, sizeof(serverSockAddr)) != 0)
    return herror_new("hsocket_open", HSOCKET_ERROR_CONNECT,
                      "Socket error (%d) (%s)", errno,strerror(errno));
...
_hsocket_sys_close(hsocket_t * sock)
{
  ...
  shutdown(sock->sock, SHUT_RDWR_SOC);
...
hsocket_nsend(hsocket_t * sock, const byte_t *bytes, int n)
{
  ...
  while (1)
  {
    char * byte_pointer = (char*)bytes + total;
    if ((status = hssl_write(sock, byte_pointer , n, &size)) != H_OK)
    {
...
{
  return hsocket_nsend(sock, (byte_t*) str, strlen(str));
}
...
/* log_verbose3("Entering hsocket_read(total=%d,force=%d)", total, force); */
  ...
  gettimeofday(&begin,NULL);	// Gestion timeout JPS - tag
  ...
  do
  {
	...
	else
	{
		...
		iCalcul = end.tv_sec-begin.tv_sec;
		if (iCalcul >= httpd_get_timeout())
		{
			// + de _httpd_timeout secondes, on sort
			errno = ETIMEDOUT;
			log_verbose2("Socket %d timeout read", sock);
			//printf(stderr,">>> ERROR LIBSOAP >>> Timeout\n");
			return -1;
		}
		// Gestion timeout JPS - tag
	}
