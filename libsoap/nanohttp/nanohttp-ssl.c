...
_hssl_server_context_init(void)
{
  ...
  //if (!enabled || !certificate)	// 20171009 Adaptation SSL SHA2
  if (!enabled)
	return H_OK;
  ...
  if (!certificate)   // 20171009 Adaptation SSL SHA2
     return H_OK;
...
hssl_client_ssl(hsocket_t * sock)
{
  SSL *ssl;
  int ret;
  SSL_METHOD* method = NULL;	// 20171009 Adaptation SSL SHA2

  log_verbose1("Starting SSL client initialization");

  _hssl_library_init();		// 20171009 Adaptation SSL SHA2
  ...
  // 20171009 Adaptation SSL SHA2
  method = SSLv23_client_method(); /* Use SSL or TLS */
